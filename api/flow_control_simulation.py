...
# Define vortices
vortices = [
    [np.array([20,30]), 10, -100],
    [np.array([20,80]), 10, -100],
    [np.array([60,80]), 10, -100],
    [np.array([80,60]), 10, -100],
]
...
ani_writer.setup(fig, "video.mp4", dpi=200)   
...
while rho > 1:
    *pcurr, _ = sim.getObjectPosition(robot_handle)
    ...
    q = np.array(pcurr[:2]) * 4 # Multiplied by 4 due to the used scene
                                # have 1/4 of the size of the grid
    ...
    cosine = (
        np.dot(desired_direction, fl[0])
        / (np.linalg.norm(desired_direction)
        * np.linalg.norm(fl[0]))
    )
    angle = np.arccos(cosine)

    at = katt * desired_direction - angle*fl[0]
